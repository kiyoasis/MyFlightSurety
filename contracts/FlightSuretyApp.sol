...
contract FlightSuretyApp {
    ...
    */   
    function registerAirline(address newAirline, string name) external requireIsOperational payable returns(bool success, uint256 votes) {
        ...
        return (success, consensus[newAirline].numOfVotes);
    }
    ...
    function isAirlineVoted(address airline) public view returns(bool) {
        ...
        for(uint c = 0; c < consensus[airline].multiCalls.length; c++) {
            if (consensus[airline].multiCalls[c] == msg.sender) {
                isDuplicate = true;
                break;
            }
        }
...
// region ORACLE MANAGEMENT
    ...
    function registerOracle() external payable returns (uint, uint, uint) {
